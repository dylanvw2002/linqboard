...
export const SimpleTaskCard = ({
  ...
  return (
    ...
    >
      <h4 className="font-semibold text-xl text-foreground mb-2 leading-snug relative z-10">
        {title}
      </h4>
      
      {description && (
        <p className="text-lg text-muted-foreground mb-3 relative z-10">
          {description}
        </p>
      )}
      
      {dueDate && (
        <div className="flex items-center gap-2 text-lg text-muted-foreground relative z-10">
        ...
        <div className="flex items-center gap-1 mt-4 relative z-[60]">
            ...
            return (
              <Tooltip key={assignee.user_id} delayDuration={200}>
                <TooltipTrigger asChild>
                  <Avatar className="h-12 w-12 border-[3px] border-white dark:border-gray-700 cursor-pointer hover:scale-110 hover:z-10 transition-transform duration-100 will-change-transform transform-gpu shadow-lg" style={{ marginLeft: idx > 0 ? '-16px' : '0' }}>
                    <AvatarImage src={assignee.avatar_url || undefined} />
                    <AvatarFallback className="text-lg font-black bg-gradient-to-br from-primary to-primary/70 text-white">
          ...
          {assignees.length > 3 && (
            <Tooltip delayDuration={200}>
              <TooltipTrigger asChild>
                <div className="h-12 w-12 rounded-full bg-gradient-to-br from-primary to-primary/70 border-[3px] border-white dark:border-gray-700 flex items-center justify-center text-lg font-black text-white cursor-pointer hover:scale-110 transition-transform duration-100 will-change-transform transform-gpu shadow-lg" style={{ marginLeft: '-16px' }}>
                  +{assignees.length - 3}
                </div>
              </TooltipTrigger>
              <TooltipContent side="top" className="z-[70] bg-foreground text-background">
