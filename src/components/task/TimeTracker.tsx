...
export const TimeTracker = ({ taskId }: TimeTrackerProps) => {
  ...
  const loadTotalTime = async () => {
    ...
    const total = (data as any[]).reduce((sum, entry) => sum + (entry.duration_seconds || 0), 0);
    setTotalTime(total);
  };
  ...
  const checkActiveEntry = async () => {
    ...
    const elapsed = Math.floor(
      (new Date().getTime() - new Date(entry.start_time).getTime()) / 1000
    );
  ...
  const startTracking = async () => {
    ...
    setCurrentEntryId((data as any).id);
    setIsTracking(true);
    setElapsedSeconds(0);
    startTimer();
  };
