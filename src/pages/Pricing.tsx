...
const Pricing = () => {
  ...
  const handleSubscribe = async (plan: Plan) => {
    ...
    // Handle downgrade (to non-free plan) - also go to checkout
    if (planLevel < currentLevel) {
      const confirmed = window.confirm(t('pricing.confirmDowngrade'));
      if (!confirmed) return;
    }
    
    // Navigate to checkout page for new subscriptions/upgrades/downgrades
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4 lg:py-8 max-w-7xl">
        ...
        <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-4 md:gap-6 lg:gap-8 max-w-7xl mx-auto">
          {isLoading ?
        // Loading skeletons
        Array.from({
          length: 4
